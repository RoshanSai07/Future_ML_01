...
xgb_df["IsMonthStart"] = (
    xgb_df["Date"].dt.is_month_start
).astype(int)

xgb_df["IsMonthEnd"] = (
    xgb_df["Date"].dt.is_month_end
).astype(int)

xgb_df["Day_Sin"] = np.sin(
    2 * np.pi * xgb_df["DayOfWeek"] / 7
)

xgb_df["Day_Cos"] = np.cos(
    2 * np.pi * xgb_df["DayOfWeek"] / 7
)

for lag in [1, 2, 3, 7, 14, 21, 28]:

    xgb_df[f"Revenue_Lag_{lag}"] = (
        xgb_df["Revenue"].shift(lag)
    )

for lag in [1, 7, 14]:

    xgb_df[f"Orders_Lag_{lag}"] = (
        xgb_df["Orders"].shift(lag)
    )

    xgb_df[f"Customers_Lag_{lag}"] = (
        xgb_df["Customers"].shift(lag)
    )

    xgb_df[f"Quantity_Lag_{lag}"] = (
        xgb_df["Quantity"].shift(lag)
    )

    xgb_df[f"AOV_Lag_{lag}"] = (
        xgb_df["AOV"].shift(lag)
    )

xgb_df["Revenue_Rolling_7"] = (
    xgb_df["Revenue"]
    .shift(1)
    .rolling(7)
    .mean()
...
sarima_pred = np.array(
    sarima_pred[-common_len:]
)

auto_pred = np.array(
    auto_pred[-common_len:]
)
...
if best_model_name == "SARIMA":
    ...
    future_values = final_fit.forecast(
        future_days
    )
    ...
    future_values = final_fit.predict(
        n_periods=future_days
    )

    joblib.dump(
        final_fit,
        "models/autoarima.pkl"
    )
    ...
    for future_date in future_dates:
        ...
        row["Revenue_Rolling_7"] = (
            history["Revenue"]
            .tail(7)
            .mean()
        )

        row["Revenue_Rolling_14"] = (
            history["Revenue"]
            .tail(14)
            .mean()
        )

        row["Revenue_Rolling_30"] = (
            history["Revenue"]
            .tail(30)
            .mean()
        )
        ...
        history = pd.concat([
            ...
            pd.DataFrame([{
                "Date": future_date,
                "Revenue": pred,
                "Orders": history["Orders"].tail(7).mean(),
                "Customers": history["Customers"].tail(7).mean(),
                "Products": history["Products"].tail(7).mean(),
                "Quantity": history["Quantity"].tail(7).mean(),
                "AOV": history["AOV"].tail(7).mean()
            }])
        ], ignore_index=True)
...
future_forecast = pd.DataFrame({
    ...
    "Date": future_dates,
    "Forecasted Revenue": future_values
